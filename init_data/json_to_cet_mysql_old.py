...
MYSQL_CONFIG = {
    ...
    "port": 3306,
    "user": "root",
    "password": "020708",  # ⚠️ 改成你的真实密码
    "database": "word_origin",
    "charset": "utf8mb4"
}

# 2. JSON文件路径（支持多个文件批量导入）
JSON_FILE_PATHS = [
    r"D:\github_project\backend\cet-word-api\data\CET4.json",
    # r"C:\Users\Administrator\Downloads\CET6\CET6.json",  # 如需导入CET6，取消注释
]

# 3. 目标表前缀（CET4填cet4，CET6填cet6）
TABLE_PREFIX = "cet4"
...
def parse_word_json(file_path: str) -> List[Dict]:
    """
        ...
        with open(file_path, "r", encoding="utf8") as f:
            for line in f:
                ...
                try:
                    ...
                    # 使用 safe_get_nested 处理可能不存在的嵌套路径
                    ...
                    parsed_words.append({
                        ...
                        # 新增字段
                        'phone': phone,
                        'star': star if star else 0,  # 确保是整数
                        'picture': picture,
                        'speech': speech,
                        # 子表数据
...
def insert_word_to_mysql(conn, cursor, word_data: Dict, table_prefix: str) -> bool:
    """
    插入单个单词及其所有关联数据到数据库
    包含完整的事务处理和错误回滚
    """
    main_table = f"{table_prefix}_words"
    
    try:
        # ==================== 1. 插入主表 ====================
        insert_main_sql = f"""
            INSERT INTO {main_table} (
                word_rank, head_word, book_id, word_head, word_id,
        ...
        cursor.execute(insert_main_sql, (
            word_data['word_rank'], word_data['head_word'], word_data['book_id'],
            word_data['word_head'], word_data['word_id'],
            word_data['us_phone'], word_data['uk_phone'],
            word_data['us_speech'], word_data['uk_speech'],
            word_data['phone'], word_data['star'], 
            word_data['picture'], word_data['speech']
        ))
        ...
        if main_id == 0:
            cursor.execute(f"""
                ...
                WHERE head_word = %s AND book_id = %s
            """, (word_data['head_word'], word_data['book_id']))
        ...
        if word_data['trans']:
            insert_trans_sql = f"""
                INSERT INTO {table_prefix}_word_trans (
        ...
        if sentences:
            insert_sent_sql = f"""
                INSERT INTO {table_prefix}_word_sentences (
        ...
        if real_exam_sentences:
            insert_real_exam_sql = f"""
                INSERT INTO {table_prefix}_word_real_exam_sentences (
        ...
        if phrases:
            insert_phrase_sql = f"""
                INSERT INTO {table_prefix}_word_phrases (
        ...
        if synos:
            insert_syno_sql = f"""
                INSERT INTO {table_prefix}_word_synos (
        ...
        if rels:
            insert_root_sql = f"""
                INSERT INTO {table_prefix}_word_roots (
        ...
        if word_data['rem_method']:
            insert_rem_sql = f"""
                INSERT INTO {table_prefix}_word_rem_methods (
        ...
        if exams:
            insert_exam_sql = f"""
                INSERT INTO {table_prefix}_word_exams (
                    word_main_id, question, exam_type, right_index, answer_explain
                ) VALUES (%s, %s, %s, %s, %s);
            """
            insert_choice_sql = f"""
                INSERT INTO {table_prefix}_word_exam_choices (
...
def process_all_files():
    ...
    for file_path in JSON_FILE_PATHS:
        words = parse_word_json(file_path)
    ...
    # 去重（基于head_word和book_id）
    unique_words = {}
    for word in all_words:
        key = (word['head_word'], word['book_id'])
        if key not in unique_words:
            unique_words[key] = word
    
    all_words = list(unique_words.values())
    print(f"📊  去重后共 {len(all_words)} 个唯一单词待导入")
    ...
    try:
        ...
        print(f"🚀  开始导入数据到MySQL，目标前缀：{TABLE_PREFIX}")
        
        for idx, word in enumerate(all_words, 1):
            try:
                conn.begin()
                insert_word_to_mysql(conn, cursor, word, TABLE_PREFIX)
                conn.commit()
                success_count += 1
                
                if idx % 100 == 0 or idx == len(all_words):
                    print(f"⏳  进度：{idx}/{len(all_words)} ({success_count}成功/{fail_count}失败)")
                    
            except Exception as e:
                conn.rollback()
                fail_count += 1
                print(f"⚠️  第{idx}个单词 '{word.get('head_word', 'unknown')}' 处理失败：{str(e)}")
                continue
        
        print(f"\n✅  导入完成！总计：{len(all_words)}，成功：{success_count}，失败：{fail_count}")
